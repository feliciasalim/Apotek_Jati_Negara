...
<body>
    ...
    <div class="title2">
            <p class="subtitle pt1" style="padding-top: 20px;">Semua &nbsp</p> 
            <p class="subtitle pt2" style="padding-top: 20px;">Produk</p>
    </div>
    <div class="subpage4" id="searchResults">
            <main class="grid">
        <?php
        $limit = 30;
        ...
        if(isset($_GET['query'])) {
            ...
            $query2 = "SELECT * FROM produk WHERE status_produk = 'Ada' AND nama_produk LIKE '%$search_query%' LIMIT $halaman_awal, $limit";
        
            $query = mysqli_query($conn, "SELECT * FROM produk WHERE status_produk = 'Ada' AND nama_produk LIKE '%$search_query%'");
            $jlh_data = mysqli_num_rows($query);
        }
        else {
            $query2 = "SELECT * FROM produk WHERE status_produk = 'Ada' LIMIT $halaman_awal, $limit";
        
            $query = mysqli_query($conn, "SELECT * FROM produk WHERE status_produk = 'Ada'");
            $jlh_data = mysqli_num_rows($query);
        }       
    ...
    $queryParameter = isset($_GET['query']) ? "&query=$search_query" : "";
